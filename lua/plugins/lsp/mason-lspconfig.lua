return {
	-- Mason Plugin
	{
		...
		config = function()
			-- Configuração principal do Mason
			require("mason").setup({
				ui = {
					icons = {
						package_installed = "✓",
			...
			-- Configuração do Mason DAP para depuração
	...
	},
	{
		...
		config = function()
			...
			local remaps = require("plugins.lsp.utils.remaps")
			...
			if present_mason then
				...
				mason.setup_handlers({
					function(server)
						local merged_config =
							vim.tbl_deep_extend("force", default_lsp_config, server_configs[server] or {})
