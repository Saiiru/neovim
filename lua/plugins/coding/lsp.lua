return {
	{
		...
		config = function()
			...
			clangd_capabilities.offsetEncoding = nil
			clangd_capabilities.offset_encodings = nil

			vim.lsp.handlers["workspace/executeCommand"] = function(
				_,
				result,
				ctx,
				_
			)
				local command = result and result.command
					or ctx and ctx.params and ctx.params.command
				if command == "setContext" then
					return
				end
				return vim.NIL
			end

			local default_log_message_handler =
			...
			vim.lsp.handlers["window/logMessage"] = function(
				...
				if
					params
					and params.message == "Command setContext not found"
				then
					return
				end
				return default_log_message_handler(err, params, ctx, config)
			end
			...
			local function on_attach(client, bufnr)
				...
				if client.name == "vtsls" or client.name == "ts_ls" then
					...
					client.commands.setContext = function() end
				end
				...
				if
					supports(client, "textDocument/inlayHint")
			...
			local servers = {
				...
				ts_ls = {
					...
					enabled = function()
						return not executable("vtsls")
							and has_workspace_typescript()
					end,
				},
				vue_ls = {
