...
keymap("n", "<S-q>", ":lua require('mini.bufremove').delete(0, false)<CR>", { desc = "Close current buffer", silent = true })

-- LSP-related mappings
keymap("n", "gd", vim.lsp.buf.definition, { desc = "[G]o to [D]efinition", silent = true })
keymap("n", "gr", function() vim.lsp.buf.references({ includeDeclaration = false }) end, { desc = "[G]o to [R]eferences", silent = true })
...
keymap("n", "<leader>cf", "<cmd>lua require('config.lsp.functions').format()<CR>", { desc = "[C]ode [F]ormat", silent = true })
keymap("n", "<leader>cl", "<cmd>lua vim.diagnostic.open_float({ border = 'rounded', max_width = 100 })<CR>", { desc = "[C]ode Diagnostics [L]ine", silent = true })
...
if vim.fn.has('macunix') == 1 then
  keymap("n", "gx", "<cmd>silent execute '!open ' . shellescape('<cWORD>')<CR>", { desc = "Open link in macOS", silent = true })
else
  keymap("n", "gx", "<cmd>silent execute '!xdg-open ' . shellescape('<cWORD>')<CR>", { desc = "Open link in Linux", silent = true })
end

-- Additional utility mappings
keymap("n", "<leader>vpp", ":e ~/.dotfiles/nvim/.config/nvim/init.lua<CR>", { desc = "[V]im [P]acker [P]rofile", silent = true })
...
keymap("n", "<S-q>", ":lua require('mini.bufremove').delete(0, false)<CR>", { desc = "Close buffer with mini.bufremove", silent = true })
